...
class SKA_TDNN(nn.Module):
    ...
    def __init__(self, block, C, model_scale, log_input=True, num_mels=80, num_out=192, resblock=ResBlock, context=True, pooling_type=None):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                # if aug == True: No augmentation now
                #     x = self.specaug(x)
...
def SKA_MainModel(eca_c=1024, eca_s=8, log_input=True, num_mels=80, num_out=192, pooling='CCSP'):
    model = SKA_TDNN(block=Bottle2neck, C=1024, model_scale=eca_s, log_input=log_input, num_mels=num_mels, num_out=num_out, resblock=ResBlock, context=True, pooling_type=pooling)
    return model